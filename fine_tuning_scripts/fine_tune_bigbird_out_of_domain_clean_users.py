...
file_handler = logging.FileHandler('out_of_domain_fine_tuning_big_bird_clean_users.log')
...
fine_tune_dataset, experiment_dataset = data_processor.get_train_test_datasets(split_by_chunks=False, seed=seed, sequence_length=2048, train_size=0.5)

logger.info('Split train data to fine-tune and experiment datasets')

with open('pickles/pickled_datasets/seed_42/out_of_domain_fine_tune_dataset_clean_users.pkl', 'wb') as f:
    pickle.dump(fine_tune_dataset, f)
    
logger.info('Pickled fine-tune dataset')
    
with open('pickles/pickled_datasets/seed_42/out_of_domain_experiment_dataset_clean_users.pkl', 'wb') as f:
    pickle.dump(experiment_dataset, f)
    
logger.info('Pickled experiment dataset')
...
train_dataloader = DataLoader(fine_tune_dataset, batch_size=batch_size, shuffle=True)
validation_dataloader = DataLoader(fine_tune_validation_dataset, batch_size=batch_size, shuffle=True)
...
with open('fine_tuned_models/out_of_domain_training_loss_clean_users.pkl', 'wb') as f:
    pickle.dump(training_loss, f)

with open('fine_tuned_models/out_of_domain_validation_loss_clean_users.pkl', 'wb') as f:
    pickle.dump(validation_loss, f)

logger.info('Pickled loss values')

# save model and tokenizer
model.save_pretrained('fine_tuned_models/out_of_domain_bigbird_roberta_base_clean_users')
data_processor.tokenizer.save_pretrained('fine_tuned_models/out_of_domain_bigbird_roberta_base_clean_users')
